...
| M4 QA artifacts | **in progress.** `docs/source/qa/` created - test plan, feature/requirements catalogue (`FEAT-*` / `REQ-*` for BE API, FE API, UI), test cases (`TC-*`, every existing test mapped + placeholders for gaps), requirements traceability matrix, coverage-by-feature table. Wired into the Sphinx toctree; `README.md` has a standalone "Test design & QA artifacts" section. Still to do: `pytest-cov` floor after M7, Allure taxonomy normalisation (item 68), a CI check that diffs the matrix against the catalogue (item 63). |
