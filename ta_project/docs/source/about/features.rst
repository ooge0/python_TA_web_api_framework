...
#. **Test data** comes from three interchangeable sources on purpose: inline
   ...
   :class:`~utilities.excel_data_provider.ExcelDataProvider`; and an SQLite
   database auto-created by the ``setup_database`` fixture.  Each source is
   exercised in at least one test to demonstrate the pattern.
